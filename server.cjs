...
const logDebug = (msg) => {
        ...
        try {
                    fs.appendFileSync(path.join(__dirname, 'debug.log'), formatted);
        } catch (e) {}
};
...
app.get('/api/notifications', async (req, res) => {
        try {
            ...
            query += ' ORDER BY created_at DESC';
                    ...
                    logError('Fetch Notifications Error', err);
                    res.status(500).json({ error: err.message });
        }
});

app.post('/api/notifications', async (req, res) => {
        const { user_id, title, message, type, issue_id } = req.body;
        const id = uuidv4();
        try {
                    await pool.query(
                                    'INSERT INTO notifications (id, user_id, title, message, type, issue_id, is_read, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                                    [id, user_id, title, message, type, issue_id, false, new Date()]
                                );
                    res.status(201).json({ id, user_id, title, message, type, issue_id, is_read: false });
        } catch (err) {
                    logError('Create Notification Error', err);
                    res.status(500).json({ error: err.message });
        }
});
...
app.patch('/api/notifications/:id', async (req, res) => {
        ...
        try {
                    await pool.query('UPDATE notifications SET is_read = ? WHERE id = ?', [is_read, req.params.id]);
                    res.json({ success: true });
        } catch (err) {
                    logError('Update Notification Error', err);
                    res.status(500).json({ error: err.message });
        }
});
...
app.post('/api/email/send', async (req, res) => {
        ...
        logDebug(`SIMULATED EMAIL to: ${to}, subject: ${subject}`);
